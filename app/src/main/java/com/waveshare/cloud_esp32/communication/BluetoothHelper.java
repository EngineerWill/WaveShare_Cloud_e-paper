...
//        }
        public void run() {
            ...
            while (true) {
                if(lan >=1 ){
                    byte[] buffer1 = new byte[lan];
                    System.arraycopy(buffer, 0, buffer1, 0, lan);
                    //String readMessage = new String(buffer1);
                    Message msg = new Message();
                    Bundle data = new Bundle();

                    data.putByteArray("BT_RECEIVE_DATA", buffer1);
                    msg.what = BluetoothHelper.BT_RECEIVE_DATA;
                    msg.setData(data);
